...
object Utils {
  ...
  def withPreviousAndNext[T](
    iterable: Iterable[T]
  ): List[(Option[T], T, Option[T])] = {
    if (iterable.isEmpty)
      Nil
    else {
  ...
  def adjacentPairsWithPreviousAndNext[T](
    xs: List[T]
  ): List[(Option[T], T, T, Option[T])] =
  ...
  def mkMultiset[T](set1: List[T]): Map[T, Int] =
    set1.groupBy(identity).mapValues(_.size).toMap
  ...
  def conditionallyMap[T](xs: List[T])(fn: PartialFunction[T, T]): List[T] =
    xs.map { t ⇒
      if (fn isDefinedAt t) fn(t) else t
    }
